...
class User {
  static async login(username, password, useragent) {
    ...
    const {
      pageTitle, userStatus, loggedOut, loggedIn,
    } = selector;

    // Responses
    const responses = clone(config.get('response.user.login'));
    const {
      success, invalidCreds, headerMismatch, cannotConnect, unknownError,
    } = responses;
  ...
  static async getHomeContent(useragent) {
    ...
    const {
      success, cannotConnect, unknownError,
    } = responses;

    const { courseDataSuccess, filesDataSuccess } = responses.data;
